...
-- idle status line reflects who (if anyone) is standing on the pad
...
local function drawIdle() draw(idleStatus()) end
...
local function startTrip(destId)
  ...
  if padDetector and not rider then
    draw("Step onto the pad first", colors.orange); return
  end
  local msg = { type = "ROUTE", dest = destId, trip = os.epoch("utc"), rider = rider }
  rednet.broadcast(msg, PROTO)
  activeTrip = msg; lineBusy = true
  applyRoute(destId)
  armTimeout()
  local prefix = rider and (rider .. " -> ") or ("Board " .. boardExit .. " -> ")
  draw(prefix .. nameById(destId), colors.lime)
end
...
local function handleMessage(msg)
  ...
  if msg.type == "ROUTE" then
    ...
    applyRoute(msg.dest)
    armTimeout()
    if msg.dest == STATION then
      draw("Arriving: " .. (msg.rider or "traveller"), colors.lime)
    else
...
while true do
  ...
  elseif ev == "timer" then
    ...
    elseif e[2] == presenceTimer then
      if not lineBusy then drawIdle() end             -- refresh the greeting
      armPresence()
    end
  elseif ev == "playerClick" then                     -- someone clicked the pad detector
    if not lineBusy then drawIdle() end

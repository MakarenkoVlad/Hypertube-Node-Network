...
local RPM           = 64     -- normal (non-portal) tube drive speed (2x slower than the original 128; portal mouths use MOUTH_RPM)
local MOUTH_RPM     = 32      -- portal MOUTH drive speed (absolute): kept slow so a rider exits the portal gently instead of being flung through the wall (still above the 16 RPM open threshold)
local IDLE_RPM      = 5       -- speed a CLOSED/idle tube spins at (keeps the shaft turning); MUST stay below the 16 RPM pull threshold so an idle tube never grabs a player
...
local TUNABLES = { "RPM", "MOUTH_RPM", "IDLE_RPM", "CALIBRATE_RPM", "TRIP_TIMEOUT", "TRIP_BEAT", "RELAUNCH_HOLD", "LS_INTERVAL", "BOARD_RANGE", "BOARD_HEIGHT" }
...
local TUNE_MIN = { RPM = 16, MOUTH_RPM = 16, IDLE_RPM = 0,  CALIBRATE_RPM = 1,   TRIP_TIMEOUT = 5,   TRIP_BEAT = 1,  RELAUNCH_HOLD = 1,  LS_INTERVAL = 1,   BOARD_RANGE = 1,  BOARD_HEIGHT = 1 }
local TUNE_MAX = { RPM = 256, MOUTH_RPM = 256, IDLE_RPM = 15, CALIBRATE_RPM = 256, TRIP_TIMEOUT = 600, TRIP_BEAT = 60, RELAUNCH_HOLD = 60, LS_INTERVAL = 300, BOARD_RANGE = 32, BOARD_HEIGHT = 64 }
...
do
  ...
  IDLE_RPM      = clampTune("IDLE_RPM", tune.IDLE_RPM)           or IDLE_RPM
...
if args[1] == "report" then       -- write a full diagnostic snapshot to a file you can send for debugging
  ...
  add(("  RPM=%s MOUTH_RPM=%s IDLE_RPM=%s CALIBRATE_RPM=%s TRIP_TIMEOUT=%s TRIP_BEAT=%s"):format(RPM, MOUTH_RPM, IDLE_RPM, CALIBRATE_RPM, TRIP_TIMEOUT, TRIP_BEAT))
...
local function gateToward(nb)                  -- spin only the owned tube to nb (nil = stop owned)
  for _, c in ipairs(ctrls) do
    if LINKS[c.name] then                      -- skip controllers we don't own
      pcall(c.wrap.setTargetSpeed, (nb ~= nil and LINKS[c.name] == nb) and (MODE == "mouth" and MOUTH_RPM or RPM) or IDLE_RPM)
    end
  end
end
